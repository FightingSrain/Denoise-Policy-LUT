...
class TotalLoss(nn.Module):
    ...
    def forward(self, out_image, gt_image):
        out_lab = self.rgb2lab(torch.Tensor(out_image))
        gt_lab = self.rgb2lab(torch.Tensor(gt_image))

        # loss = F.l1_loss(out_lab[:, 1:3], gt_lab[:, 1:3]) * 127
        # loss += F.l1_loss(out_lab[:, 0:1], gt_lab[:, 0:1]) * 100
        # loss = F.l1_loss(out_lab[:, 1:3], gt_lab[:, 1:3]) * 1
        # loss += F.l1_loss(out_lab[:, 0:1], gt_lab[:, 0:1]) * 1

        loss = F.mse_loss(out_lab[:, 1:3], gt_lab[:, 1:3]) * 127
        loss += F.mse_loss(out_lab[:, 0:1], gt_lab[:, 0:1]) * 100

        return loss, out_lab, gt_lab
