...
class masksamplingv2(nn.Module):
    ...
    def forward(self, input, patten):
        ...
        if patten == 0:
            ...
            return output2.numpy(), output1.numpy()
        elif patten == 1:
            output1 = self.pool(input)
            output2 = self.pool(input[:, :, 1:, :])
            return output2.numpy(), output1.numpy()
            ...
            return output1.numpy(), output3.numpy()
        elif patten == 3:
            output1 = self.pool(input)
            output3 = self.pool(input[:, :, :, 1:])
            return output3.numpy(), output1.numpy()
        elif patten == 4:
            output2 = self.pool(input[:, :, 1:, :])
            output4 = self.pool(input[:, :, 1:, 1:])
            return output4.numpy(), output2.numpy()
        elif patten == 5:
            output2 = self.pool(input[:, :, 1:, :])
            output4 = self.pool(input[:, :, 1:, 1:])
            return output2.numpy(), output4.numpy()
        elif patten == 6:
            output3 = self.pool(input[:, :, :, 1:])
            output4 = self.pool(input[:, :, 1:, 1:])
            return output3.numpy(), output4.numpy()
        elif patten == 7:
            output3 = self.pool(input[:, :, :, 1:])
            output4 = self.pool(input[:, :, 1:, 1:])
            return output4.numpy(), output3.numpy()
        else:
...
#         print(redinput.shape)
#         print(blueinput.shape)