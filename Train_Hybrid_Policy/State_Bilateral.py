    ...
    def step(self, act, inner_state):
        ...
        for i in range(0, b):
            if np.sum(act[i] == self.move_range) > 0:
                gaussian[i] = np.expand_dims(cv2.bilateralFilter(self.image[i].squeeze().astype(np.float32), d=5,
            ...
            if np.sum(act[i] == self.move_range + 1) > 0:
                bilateral[i] = np.expand_dims(cv2.bilateralFilter(self.image[i].squeeze().astype(np.float32), d=5,
                                                                  sigmaColor=0.3, sigmaSpace=5), 0)
            if np.sum(act[i] == self.move_range + 2) > 0:
                median[i] = np.expand_dims(cv2.bilateralFilter(self.image[i].squeeze().astype(np.float32), d=5,
                                                                  sigmaColor=0.5, sigmaSpace=5), 0)

            if np.sum(act[i] == self.move_range + 3) > 0:
                gaussian2[i] = np.expand_dims(cv2.bilateralFilter(self.image[i].squeeze().astype(np.float32), d=5,
                                                                  sigmaColor=0.7, sigmaSpace=5), 0)
            if np.sum(act[i] == self.move_range + 4) > 0:
                bilateral2[i] = np.expand_dims(cv2.bilateralFilter(self.image[i].squeeze().astype(np.float32), d=5,
                                                                  sigmaColor=0.9, sigmaSpace=5), 0)
            if np.sum(act[i] == self.move_range + 5) > 0:  # 7
                box[i] = np.expand_dims(cv2.bilateralFilter(self.image[i].squeeze().astype(np.float32), d=5,
                                                                  sigmaColor=1.1, sigmaSpace=5), 0)
